"""
Breast Cancer Dataset - Professional Exploratory Data Analysis (EDA)
Enhanced version with parametric + non-parametric statistics and statistical recommendations.
...
SEVERE_IMBALANCE_RATIO = 3.0  # >3:1 or <1:3 considered severe
MODERATE_IMBALANCE_RATIO = 1.5  # >1.5:1 or <1:1.5 considered moderate

# Cardinality thresholds
HIGH_CARDINALITY_THRESHOLD = 1000  # >1000 unique values considered high cardinality
WARNING_CARDINALITY_THRESHOLD = 500  # >500 unique values triggers warning


# ============================================================================
# STATISTICAL ADVISOR CLASS (NEW)
# ============================================================================
...
class StatisticalAdvisor:
    ...
    def _check_completeness(self) -> None:
        ...
        # This will be set from outside; for now, placeholder.
    ...
    def _generate_recommendation(self) -> None:
        ...
        if not is_normal or outlier_severity in ['moderate', 'severe']:
            ...
            # Mild outliers - robust methods recommended but parametric possible
            preferred = 'robust'
            reasoning = f"mild outliers present ({outliers.get('iqr_pct', 0):.1f}%) - consider both methods"
            suggested_corr = 'spearman'  # still prefer non-parametric
            suggested_test = 'mannwhitneyu'
        else:
            # Normal and no outliers
...
class DataLoader:
    ...
    def validate_columns(self, expected_target: str = 'diagnosis') -> None:
        ...
        if expected_target not in self.df.columns:
    ...
    def get_basic_info(self) -> Dict[str, Any]:
        ...
        if 'diagnosis' in self.df.columns:
            target_counts = self.df['diagnosis'].value_counts()
            target_dist = target_counts.to_dict()
            
            if 'B' in target_counts and 'M' in target_counts:
                n_benign = target_counts['B']
                n_malignant = target_counts['M']
                ratio = max(n_benign, n_malignant) / min(n_benign, n_malignant)
                
                if ratio >= SEVERE_IMBALANCE_RATIO:
                    imbalance_warning = f"SEVERE imbalance: {n_benign} B : {n_malignant} M (ratio {ratio:.2f}:1)"
                    modeling_suggestions = [
                    ...
                    imbalance_warning = f"MODERATE imbalance: {n_benign} B : {n_malignant} M (ratio {ratio:.2f}:1)"
                    modeling_suggestions = [
        ...
        # Cardinality detection - overall dataset summary
        cardinality_summary = {}
...
# ============================================================================
# COLUMN ANALYZER CLASS (ENHANCED with non-parametric methods + categorical stats)
# ============================================================================

class ColumnAnalyzer:
    """
    Performs rigorous analysis on a single numeric column:
    ...
    def analyze(self) -> Dict[str, Any]:
        ...
        # Add cardinality detection for all column types
        ...
        if self.series.dtype.kind in 'iufc':
            ...
            # Basic numeric stats (parametric + robust)
            ...
            # Correlations with target (if available)
            if self.target is not None and self.target.dtype in ['object', 'category']:
                results["target_comparison"] = self._compare_by_target()
                results["correlations_with_target"] = self._compute_correlations_with_target()
        
        # Categorical-specific stats (NEW)
            ...
            # For categorical, also check cardinality warning in log
            if results["cardinality"]["is_high_cardinality"]:
    ...
    def _compute_correlations_with_target(self) -> Dict[str, Any]:
        ...
        target_encoded = self.target.map({'M': 1, 'B': 0})
    ...
    def _analyze_categorical(self) -> Dict[str, Any]:
        ...
        - Chi-square test vs target (if target is binary)
        - Cramér's V (effect size)
        - Entropy (information content)
        """
        results = {
        ...
        # Chi-square test with target (only if target exists and is binary categorical)
        if self.target is not None and self.target.dtype in ['object', 'category']:
            chi2_result = self._chi_square_vs_target()
            results["chi_square_vs_target"] = chi2_result
            
            if chi2_result and chi2_result.get('p_value') is not None:
    ...
    def _compute_entropy(self) -> float:
        ...
        Lower entropy = dominated by few categories.
        """
...
class Visualizer:
    ...
    def plot_numeric_distribution(self, series: pd.Series, col_name: str, target: Optional[pd.Series] = None) -> str:
        ...
        if target is not None:
            ...
            ax1.set_title(f'{col_name} - Boxplot by Diagnosis')
            ax1.set_xlabel('Diagnosis (M=Malign, B=Benign)')
            ax1.set_ylabel(col_name)
            
            sns.histplot(data=df_temp, x="value", hue="target", kde=True, ax=ax2, alpha=0.6)
            ax2.set_title(f'{col_name} - Distribution by Diagnosis')
    ...
    def plot_correlation_heatmap(self, df: pd.DataFrame, target_col: str = 'diagnosis') -> str:
        """Plot correlation matrix including point-biserial correlation with target."""
        numeric_df = df.select_dtypes(include=[np.number])
        
        if target_col in df.columns:
            target_encoded = df[target_col].map({'M': 1, 'B': 0})
            numeric_df = numeric_df.copy()
            numeric_df['diagnosis_encoded'] = target_encoded
    ...
    def plot_top_correlations(self, df: pd.DataFrame, target_col: str = 'diagnosis', top_k: int = 10) -> str:
        """Plot top K features most correlated with diagnosis using Spearman (non-parametric)."""
        ...
        plt.xlabel('Absolute Spearman Correlation with Diagnosis')
        plt.title(f'Top {top_k} Features Correlated with Malignancy (Non-parametric)')
    ...
    def plot_categorical_distribution(self, series: pd.Series, col_name: str, target: Optional[pd.Series] = None) -> str:
        """NEW: Plot categorical distribution with optional target breakdown."""
        fig, axes = plt.subplots(1, 2 if target is not None else 1, figsize=(14, 5))
        
        # Get top 15 categories or fewer
...
# ============================================================================
# EDA LOGGER & RESULTS SAVER (ENHANCED)
# ============================================================================
...
class EDAResultSaver:
    ...
    def save_cardinality_report(self, results: List[Dict[str, Any]], filename: str = "cardinality_report.csv") -> str:
        """NEW: Save cardinality report for all columns."""
        rows = []
        for res in results:
    ...
    def save_categorical_analysis(self, results: List[Dict[str, Any]], filename: str = "categorical_analysis.csv") -> str:
        """NEW: Save categorical column analysis (Chi-square, Cramér's V, Entropy)."""
        rows = []
        for res in results:
...
# ============================================================================
# MAIN EDA ORCHESTRATOR (ENHANCED)
# ============================================================================

class EDAReport:
    """
    Main orchestrator for the entire EDA process.
    Loads data, analyzes each column, generates plots, and saves outputs.
    """
    
    def __init__(self, data_path: str, output_base_dir: str = OUTPUT_DIR):
    ...
    def run(self) -> None:
        ...
        self.logger.info("=" * 80)
        self.logger.info("STARTING PROFESSIONAL EDA (Enhanced - Parametric + Non-parametric)")
        self.logger.info("=" * 80)
        ...
        target_series = self.df['diagnosis'] if 'diagnosis' in self.df.columns else None
        ...
        for col in self.df.columns:
            ...
            if self.df[col].dtype.kind in 'iufc' and not result.get('is_constant', False):
                visualizer.plot_numeric_distribution(self.df[col], col, target_series)
                if len(self.df[col].dropna()) > 10:
                    visualizer.plot_qq(self.df[col], col)
            elif self.df[col].dtype.kind in 'O' or pd.api.types.is_categorical_dtype(self.df[col]):
                # Generate categorical plot if not too many categories
                unique_count = self.df[col].nunique()
                if unique_count <= 50:  # Only plot if reasonable number of categories
                    visualizer.plot_categorical_distribution(self.df[col], col, target_series)
                else:
                    self.logger.info(f"Skipping categorical plot for '{col}' due to high cardinality ({unique_count} categories)")
        
        # 4. Global visualizations
        numeric_df = self.df.select_dtypes(include=[np.number])
        if numeric_df.shape[1] > 1:
            visualizer.plot_correlation_heatmap(self.df, 'diagnosis')
            visualizer.plot_top_correlations(self.df, 'diagnosis')
        ...
        self._print_summary(basic_info)
        ...
        self.logger.info("    * cardinality_report.csv - NEW: Cardinality analysis per column")
        self.logger.info("    * categorical_analysis.csv - NEW: Chi-square, Cramér's V, entropy")
        self.logger.info("  - Plots: plots/")
        self.logger.info("  - Logs: logs/")
        self.logger.info("=" * 80)
    
    def _print_summary(self, basic_info: Dict) -> None:
        ...
        # Statistical recommendations summary
        ...
        if categorical_cols:
            ...
            sig_assoc = sum(1 for r in categorical_cols 
                          if r.get('categorical_stats', {}).get('chi_square_vs_target', {}).get('significant', False))
...
if __name__ == "__main__":
    ...
    if not os.path.exists(DATA_FILE):
        ...
        print("Asegúrate de que el archivo CSV está en el mismo directorio que este script.")
        sys.exit(1)
    
    eda = EDAReport(DATA_FILE)
    eda.run()
