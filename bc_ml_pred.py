"""
Dataset de Cáncer de Seno - Modelado Predictivo con Random Forest vs XGBoost
Autor: Gutierrez-Escobar-AJ
Propósito: Comparar Random Forest y XGBoost para la clasificación de cáncer de seno
...
# Feature selection
TOP_FEATURES = 15  # Number of top features to select (None for all)

# GridSearchCV configuration
USE_GRID_SEARCH = True  # Set to False to use default parameters
GRID_SEARCH_CV = 3  # Inner CV folds for GridSearch (reduced for speed)
...
def setup_logging() -> logging.Logger:
    ...
    log_file = os.path.join(OUTPUT_DIR, SUBDIRS["logs"],
                            f"ml_models_{datetime.now().strftime('%Y%m%d_%H%M%S')}.log")
...
class DataPreprocessor:
    """Handle data loading, preprocessing, and feature selection."""
    ...
    def load_data(self) -> None:
        ...
        self.logger.info("=" * 80)
        self.logger.info("DATA LOADING & PREPROCESSING")
        self.logger.info("=" * 80)
        ...
        try:
            ...
            self.y = self.df["diagnosis"].map({"B": 0, "M": 1})

            # Remove ID column if present
            if "id" in self.X.columns:
                self.X = self.X.drop(columns=["id"])
                self.logger.info("Removed 'id' column (unique identifier)")
            ...
            n_benign = (self.y == 0).sum()
            n_malignant = (self.y == 1).sum()
            ratio = max(n_benign, n_malignant) / min(n_benign, n_malignant)
            self.logger.info(f"Class imbalance ratio: {ratio:.2f}:1")
            if ratio > 1.5:
                self.logger.warning("Moderate to severe imbalance detected. Using class weights.")
...
class GridSearchOptimizer:
    ...
    def optimize_random_forest(self, X: np.ndarray, y: np.ndarray) -> Tuple[RandomForestClassifier, Dict]:
        ...
        # Create model with best parameters and fit it on full training data
    ...
    def optimize_xgboost(self, X: np.ndarray, y: np.ndarray):
        ...
        # Calculate scale_pos_weight for imbalance (fixed, not optimized)
        n_benign = (y == 0).sum()
        n_malignant = (y == 1).sum()
...
class ModelEvaluator:
    ...
    def plot_roc_curves(self, test: bool = False) -> str:
        ...
        filepath = os.path.join(self.output_dir, f"roc_curves_comparison{suffix}.png")
    ...
    def plot_confusion_matrices(self, test: bool = False) -> str:
        ...
        plt.tight_layout()
        filepath = os.path.join(self.output_dir, f"confusion_matrices{suffix}.png")
        plt.savefig(filepath, dpi=150, bbox_inches='tight')
        plt.close()
    ...
    def generate_classification_reports(self, output_dir: str, test: bool = False) -> pd.DataFrame:
        ...
        if not df_reports.empty:
            filepath = os.path.join(output_dir, f"classification_reports{suffix}.csv")
...
class FeatureImportanceAnalyzer:
    ...
    def plot_importance(self, importance_dict: Dict[str, pd.DataFrame], top_k: int = 15) -> str:
        ...
        for idx, (model_name, df_imp) in enumerate(importance_dict.items()):
            ...
            axes[idx].set_title(f'{model_name} - Top {top_k} Features')
            axes[idx].invert_yaxis()

        plt.tight_layout()
        filepath = os.path.join(self.output_dir, "feature_importance_comparison.png")
        plt.savefig(filepath, dpi=150, bbox_inches='tight')
        plt.close()
    ...
    def save_importance(self, importance_dict: Dict[str, pd.DataFrame], output_dir: str) -> None:
        ...
        for model_name, df_imp in importance_dict.items():
            ...
            filepath = os.path.join(output_dir, f"feature_importance_{safe_name}.csv")
            df_imp.to_csv(filepath, index=False)
            self.logger.info(f"Saved feature importance for {model_name}: {filepath}")


# ============================================================================
# MODEL COMPARATOR (Statistical Tests)
# ============================================================================
...
class ModelComparator:
    ...
    def compare_metrics(self, test: bool = False) -> pd.DataFrame:
        ...
        filepath = os.path.join(self.output_dir, f"model_comparison_{suffix}.csv")
    ...
    def mcnemar_test(self, test: bool = False) -> Dict:
        ...
        filepath = os.path.join(self.output_dir, f"mcnemar_test_{dataset}.csv")
...
class LearningCurveAnalyzer:
    ...
    def plot_learning_curves(self, model, X: np.ndarray, y: np.ndarray,
        ...
        filepath = os.path.join(self.output_dir, f"learning_curves_{model_name.lower().replace(' ', '_')}.png")
        plt.savefig(filepath, dpi=150, bbox_inches='tight')
        plt.close()
...
class HyperparameterSummary:
    ...
    def save_summary(self) -> str:
        ...
        if rows:
            ...
            filepath = os.path.join(self.output_dir, "best_hyperparameters.csv")
...
class MLReport:
    """Main orchestrator for predictive modeling comparison."""
    ...
    def run(self) -> None:
        ...
        self.logger.info("=" * 80)
        self.logger.info("PREDICTIVE MODELING: Random Forest vs XGBoost")
        if USE_GRID_SEARCH:
        ...
        # 5. Feature importance (models already fitted during training)
        ...
        # 7. Learning curves (using best models on full training data)
    ...
    def _print_final_recommendation(self) -> None:
        ...
        # Compare test ROC-AUC as primary metric
        best_model = None
        best_auc = -1
        ...
        if 'Random Forest' in results and 'XGBoost' in results:
            ...
            if rf_auc > xgb_auc + 0.01:
                ...
                self.logger.info("  → More robust to outliers and non-linear relationships")
            elif xgb_auc > rf_auc + 0.01:
                self.logger.info("XGBoost outperforms Random Forest on test data")
                self.logger.info("  → Better handling of feature interactions")
            else:
                self.logger.info("Both models perform similarly on test data")
                self.logger.info("  → Consider ensemble or simpler model for deployment")

        self.logger.info("")
        self.logger.info("Next steps:")
        self.logger.info("  1. Fine-tune hyperparameters further with more iterations")
        self.logger.info("  2. Test on external validation dataset")
        self.logger.info("  3. Consider ensemble of both models (stacking)")
        self.logger.info("  4. Deploy with probability calibration if needed")
        if USE_GRID_SEARCH:
            self.logger.info("  5. Try RandomizedSearchCV for larger parameter spaces")
